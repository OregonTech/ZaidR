...
from git_utils import get_file_info, get_git_file_info,is_file_in_repo


class TestFilePath(TestCase):
    @requirements(['#00100'])
    @patch('git_utils.git_execute')
    def test_true_Path(self, git_execute_mock):
        m = git_execute_mock()
        m.return_value = 'No'
        p = urlparse.urlparse('file://C:/Users/Zed/Desktop/CST 236 Lab 4 - Copy/cst236_lab4/output.txt')
        filepath = os.path.abspath(os.path.join(p.netloc, p.path))
        self.assertTrue(is_file_in_repo(filepath))
    @patch('git_utils.git_execute')
    def test_false_Path(self, git_execute_mock):
        m = git_execute_mock()
        m.return_value = True
        p = urlparse.urlparse('file://C:/Users/Zed/Desktop/CST 236 Lab 4 - Copy/cst236_lab4/output.txt')
        filepath = os.path.abspath(os.path.join(p.netloc, p.path))
    ...
    @requirements(['#00110'])
    @patch('git_util.git_execute')
    def test_file_status_modified(self, git_execute_mock):
        m = git_execute_mock()
        m.return_value = '%s is modified locally'
        p = urlparse.urlparse('file://C:/Users/Zed/Desktop/CST 236 Lab 4 - Copy/cst236_lab4/output.txt')
        filepath = os.path.abspath(os.path.join(p.netloc, p.path))
        self.assertEqual(get_git_file_info(filepath), '{} has been modified locally')
    @patch('git_util.git_execute')
    def test_file_status_checked(self, git_execute_mock):
        m = git_execute_mock()
        m.return_value = '%s is not checked in'
        p = urlparse.urlparse('file://C:/Users/Zed/Desktop/CST 236 Lab 4 - Copy/cst236_lab4/output.txt')
        filepath = os.path.abspath(os.path.join(p.netloc, p.path))
        self.assertEqual(get_git_file_info(filepath), 'is not checked in')
    @patch('git_util.git_execute')
    def test_file_status_in_dirty_repo(self, git_execute_mock):
        m = git_execute_mock()
        m.return_value = '%s is contained in a dirty repo'
        p = urlparse.urlparse('file://C:/Users/Zed/Desktop/CST 236 Lab 4 - Copy/cst236_lab4/output.txt')
        filepath = os.path.abspath(os.path.join(p.netloc, p.path))
        self.assertEqual(get_git_file_info(filepath), 'is contained in a dirty repo')

    @requirements(['#00120'])
    @patch('git_util.git_execute')
    def test_file_deal(self, git_execute_mock):
        m = git_execute_mock()
        m.return_value = '%s is contained in a dirty repo'
        p = urlparse.urlparse('file://C:/Users/Zed/Desktop/CST 236 Lab 4 - Copy/cst236_lab4/output.txt')
        filepath = os.path.abspath(os.path.join(p.netloc, p.path))
        self.assertEqual(get_file_info(filepath), 'is contained in a dirty repo')
